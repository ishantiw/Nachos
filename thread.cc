		  ...
		  int32_t func, int arg)
{
  ...
  int8_t* KStack = AllocBoundedArray(SIMULATORSTACKSIZE);
  process->numThreads++;
  InitSimulatorContext(KStack,SIMULATORSTACKSIZE);
  InitThreadContext(func,(int32_t)stackPointer,arg);
...
{
    #ifndef ETUDIANTS_TP
    printf("**** Warning: method Thread::Finish is not implemented yet\n");
    exit(-1);
    #endif
    #ifdef ETUDIANTS_TP
