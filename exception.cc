...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr)
{
  ...
  case SYSCALL_EXCEPTION: {
	...
	case SC_SEM_CREATE: {
	 ...
	 int32_t sid;
	 Semid = new Semaphore(debug,g_machine->ReadIntRegister(5)); //creating the semaphore
     	 if(Semid) 					   //checking for any error while creating a Semaphore
	 {
	 	sid = g_object_ids->AddObject(Semid);
		g_syscall_error->SetMsg((char*)"",NoError);
		g_machine->WriteIntRegister(2,sid);					//returning the semaphore ID for other use
	 }
	 else
	 { 
		sid = g_object_ids->AddObject(Semid);
		g_syscall_error->SetMsg(msg, OutOfMemory); //showing the error message for InvalidSemaphoreId msgerror.cc
       		sprintf(msg,"%d",sid);
        	g_machine->WriteIntRegister(2,-1);
	 }
		break;
	...
	if(Sem && Sem->typeId == INVALID_TYPE_ID)//checking type id of the semaphore to a correct type 
	{
		 delete Sem;
		 g_syscall_error->SetMsg((char*)"",NoError);
		 g_machine->WriteIntRegister(2,0);
	...
	Semid = (Semaphore*)g_object_ids->SearchObject(sid);

	if (Semid->typeId == SEMAPHORE_TYPE_ID) 
	{
		Semid->V();
		g_machine->WriteIntRegister(2,0);      
	}
	...
	case SC_LOCK_CREATE:
	{
		DEBUG('e', (char*)"LOCK CREATED \n");
		Lock* lockid;int addr;
		int sizep; 
	 	addr = g_machine->ReadIntRegister(4);
	 	sizep = GetLengthParam(addr);
	 	char debug[sizep];
	 	GetStringParam(addr,debug,sizep);//Getting the first string argument
	 	lockid = new Lock(debug);
	 	int32_t lid;
	 	if(lockid)
	 	{
	 		lid = g_object_ids->AddObject(lockid);
	 		g_machine->WriteIntRegister(2,lid);//eturning the lockid
	 	}
	 	 else
	 	 {
	 	 	g_syscall_error->SetMsg(msg, InvalidLockId);//showing the error message for InvalidLockId msgerror.cc
        		//sprintf(msg,"%d",lid);
    			g_machine->WriteIntRegister(2,-1);
	 	 }
		 break;
	 ...
	 {
	 	DEBUG('e', (char*)"LOCK DESTROYED \n");
	 	Lock* lockid;
		int32_t lid;
		lid = g_machine->ReadIntRegister(4);
		lockid = (Lock*)g_object_ids->SearchObject(lid);
		if(lockid && lockid->typeId == INVALID_TYPE_ID)
		{
			delete lockid;
		 	g_syscall_error->SetMsg((char*)"",NoError);
		 	g_machine->WriteIntRegister(2,0);
	 ...
	 case SC_LOCK_ACQUIRE:
	 {
		 ...
		 lid = g_machine->ReadIntRegister(4);
		 lockid = (Lock*)g_object_ids->SearchObject(lid);
		if (lockid && lockid->typeId == LOCK_TYPE_ID) 
		{
			lockid->Acquire();
			g_machine->WriteIntRegister(2,0);      
		}
	 ...
	 case SC_COND_CREATE: 
	 {
	 	 ...
	 	 GetStringParam(addr,debug,sizep);                //Getting the first string argument
	 	 int32_t cid;
	 	 condition = new Condition(debug); //creating the Condition
         	 if(condition) 					   //checking for any error while creating a Condition
	 	 {
	 		cid = g_object_ids->AddObject(condition);
			g_syscall_error->SetMsg((char*)"",NoError);
			g_machine->WriteIntRegister(2,cid);					//returning the Condition ID for other use
	 	 }
	 	 else
	 	 {
			g_syscall_error->SetMsg(msg, OutOfMemory); //showing the error message for  msgerror.cc
        		//sprintf(msg,"%d",cid);
        		g_machine->WriteIntRegister(2,-1);
	 	 }
		 break;
	 ...
	 case SC_COND_DESTROY:
	 {
		...
		condition = (Condition*)g_object_ids->SearchObject(cid);
		if(condition && condition->typeId == INVALID_TYPE_ID)//checking type id of the condition to a correct type 
		{
			 delete condition;
		 	 g_syscall_error->SetMsg((char*)"",NoError);
		 	 g_machine->WriteIntRegister(2,0);
	 ...
	 case SC_COND_WAIT:
	 {
		...
		if (condition->typeId == CONDITION_TYPE_ID) 
		{
			condition->Wait();
			g_machine->WriteIntRegister(2,0);      
		}
